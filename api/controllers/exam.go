...
func ExamSearch() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, responses.ExamResponse{Status: http.StatusInternalServerError, Message: "error", Data: map[string]interface{}{"data": err.Error(), "filter": query}})
			return
		}
...
func ExamById() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		objId, _ := primitive.ObjectIDFromHex(examId)
		...
		// return result
		c.JSON(http.StatusOK, responses.CourseResponse{Status: http.StatusOK, Message: "success", Data: exam})
	}
}
